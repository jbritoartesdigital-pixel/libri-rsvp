...
const DEFAULT_APPEARANCE={background_color:"#f8efec",card_color:"#fffaf7",text_color:"#4f2d2a",muted_color:"#866e68",button_color:"#b8735f",button_text_color:"#ffffff",overlay_color:"#3a1f1b",overlay_opacity:.18,card_opacity:.94,card_blur:12,card_radius:28,font_style:"elegant",card_style:"glass",background_position:"center",background_x:"center",card_width:"medium",invitation_url:"",calendar_location:"",location_url:"",calendar_end_time:"",cover_url:"",logo_url:""};
...
const fmtDate=v=>{if(!v)return"Data não informada";try{return new Intl.DateTimeFormat("pt-BR",{dateStyle:"medium"}).format(new Date(`${v}T12:00:00`))}catch{return v}};
const fmtDT=v=>{if(!v)return"";try{return new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(new Date(v))}catch{return v}};
const safeAppearance=e=>({...DEFAULT_APPEARANCE,...(e?.appearance_settings||{})});
const safeTexts=e=>({...DEFAULT_TEXTS,...(e?.public_texts||{})});
const safePerms=e=>({...DEFAULT_PERMS,...(e?.client_permissions||{})});
const statusLabel=s=>s==="yes"?"Confirmado":s==="no"?"Não irá":"Aguardando";
const attendanceLabel=s=>s==="yes"?"Vai":s==="no"?"Não vai":"Aguardando";
const sourceLabel=s=>({admin:"Libri",client:"Cliente",public:"Convidado",import:"Importação"}[s]||s||"—");
...
const fmtResponseTime=v=>{
 ...
 try{
  ...
  if(dateKeySao(d)===dateKeySao(now))return`Hoje às ${time}`;
  if(dateKeySao(d)===dateKeySao(yesterday))return`Ontem às ${time}`;
  const day=new Intl.DateTimeFormat("pt-BR",{timeZone:"America/Sao_Paulo",day:"2-digit",month:"2-digit"}).format(d);
  return`${day} às ${time}`;
 }catch{return fmtDT(v)}
};
...
async function copy(text){try{await navigator.clipboard.writeText(text);toast("Link copiado.")}catch{toast("Não foi possível copiar o link.",true)}}
...
function topbar(extra=""){return`<div class="topbar"><a class="brand" href="/"><div class="mark">L</div><div><h1>Libri RSVP</h1><small>Convites virtuais e arte digital</small></div></a>${extra}</div>`}
...
const loading=()=>`<div class="card panel"><div class="loading-inline"><span class="spinner"></span>Carregando...</div></div>`;
...
function eventForm(e=null){
 const f=e?.extra_fields||{},p=safePerms(e);
...
function overview(root,e,s,info,role){
 const total=Number(s.yes_responses||0)+Number(s.no_responses||0)+Number(s.pending_responses||0),pct=total?Math.round((Number(s.yes_responses||0)+Number(s.no_responses||0))/total*100):0;
...
async function guestsTab({root,event,role,eventId,token}){
 ...
 <div class="toolbar"><div class="search-shell"><input id="search" placeholder="Digite para buscar..." autocomplete="off"><button type="button" class="search-clear" id="clearSearch" aria-label="Limpar busca" title="Limpar busca" hidden>×</button></div><div class="segmented"><button class="active" data-filter=""><span>Todos</span><b data-count="total">0</b></button><button data-filter="yes"><span>Confirmados</span><b data-count="yes">0</b></button><button data-filter="pending"><span>Aguardando</span><b data-count="pending">0</b></button><button data-filter="no"><span>Não irão</span><b data-count="no">0</b></button></div></div><div id="guestList">${loading()}</div>`;
 let q="",status="",timer;
 ...
 const refresh=async()=>{try{const d=await api(`${base}/guests?q=${encodeURIComponent(q)}&status=${status}`);updateCounts(d.counts);root.querySelector("#guestList").innerHTML=guestCards(d.guests,canManage);if(canManage)bindGuestActions(root,{event,role,eventId,token})}catch(e){root.querySelector("#guestList").innerHTML=`<div class="empty">${esc(e.message)}</div>`}};
...
function guestCards(guests,editable){
 if(!guests.length)return`<div class="empty">Nenhum convidado encontrado.</div>`;
 return`<div class="guest-list">${guests.map(g=>`<article class="guest-card${isRecentResponse(g.responded_at)?" recent-response":""}"><div class="guest-card-head"><div><div class="chips"><span class="status ${g.response_status}">${statusLabel(g.response_status)}</span>${isRecentResponse(g.responded_at)?`<span class="chip new-response">NOVA</span>`:""}${g.max_people_allowed?`<span class="chip">limite ${g.max_people_allowed}</span>`:""}</div><h3 class="guest-card-name">${esc(g.group_label||g.primary_name)}</h3>${g.group_label?`<div class="guest-card-group">Contato: ${esc(g.primary_name)}</div>`:""}</div><span class="subtle">${esc(sourceLabel(g.source))}</span></div>
 <div class="guest-card-members">${g.members.length?g.members.map(m=>`<div class="guest-member"><span class="guest-member-icon">${m.person_type==="child"?"🧒":"👤"}</span><span class="guest-member-name">${esc(m.name)}</span><span class="badge ${m.attendance_status==="yes"?"good":m.attendance_status==="no"?"bad":"pending"}">${attendanceLabel(m.attendance_status)}</span></div>`).join(""):`<div class="subtle">Nenhuma pessoa cadastrada.</div>`}</div>
 ${g.love_message?`<div class="notice" style="margin-top:10px">💌 ${esc(g.love_message)}</div>`:""}
 <div class="guest-card-footer"><div class="guest-card-contact">${g.phone?`☎ ${esc(g.phone)}`:"Sem telefone"}${g.responded_at?`<span class="response-time"> • ${esc(fmtResponseTime(g.responded_at))}</span>`:""}</div>${editable?`<div class="guest-card-actions"><button class="btn secondary small" data-edit='${esc(JSON.stringify(g))}'>Editar</button><button class="btn danger small" data-delete="${g.id}" data-name="${esc(g.group_label||g.primary_name)}">Excluir</button></div>`:""}</div></article>`).join("")}</div>`;
}
function bindGuestActions(root,ctx){
 ...
 root.querySelectorAll("[data-delete]").forEach(b=>b.onclick=async()=>{if(!confirm(`Excluir ${b.dataset.name}?`))return;const base=ctx.role==="admin"?`/api/admin/events/${ctx.eventId}`:`/api/client/${encodeURIComponent(ctx.token)}`;try{await api(`${base}/guests/${b.dataset.delete}`,{method:"DELETE",body:"{}"});toast("Convidado excluído.");ctx.role==="admin"?renderAdminEvent(ctx.eventId,"guests"):clientApp(ctx.token,"guests")}catch(e){toast(e.message,true)}})
}
function guestModal({event,role,eventId,token,guest=null,onSaved}){
 const w=modal(guest?"Editar confirmação":"Cadastrar convidado/família",`<form id="gf"><div class="field"><label>Nome da família / grupo</label><input name="group_label" placeholder="Ex.: Família Brito" value="${esc(guest?.group_label||"")}"></div><div class="field"><label>Contato principal</label><input name="primary_name" id="primary" required value="${esc(guest?.primary_name||"")}"></div><div class="row"><div class="field"><label>Telefone</label><input name="phone" value="${esc(guest?.phone||"")}"></div><div class="field"><label>Limite desta família</label><input name="max_people_allowed" type="number" min="1" max="100" placeholder="${event.max_people_per_rsvp?`Padrão: ${event.max_people_per_rsvp}`:"Sem limite"}" value="${esc(guest?.max_people_allowed??"")}"></div></div><div class="form-section"><h3>Pessoas deste convite</h3><div id="members"></div><div class="actions"><button type="button" class="btn secondary small" id="aa">+ Adulto</button><button type="button" class="btn secondary small" id="ac">+ Criança</button></div></div><details><summary>Informações opcionais</summary><div style="margin-top:10px"><div class="field"><label>Restrição alimentar</label><input name="dietary" value="${esc(guest?.dietary||"")}"></div><div class="field"><label>Observações</label><textarea name="notes">${esc(guest?.notes||"")}</textarea></div><div class="field"><label>Mensagem carinhosa 💌</label><textarea name="love_message">${esc(guest?.love_message||"")}</textarea></div></div></details><button class="btn block large" style="margin-top:16px">${guest?"Salvar alterações":"Cadastrar família"}</button></form>`,event.rsvp_mode==="list"?"Esses nomes poderão ser encontrados na busca do convite.":"Cadastre cada pessoa individualmente.",true);
 const form=w.querySelector("#gf"),root=w.querySelector("#members"),primary=w.querySelector("#primary");let touched=false;
 const add=(type,m={})=>{const row=document.createElement("div");row.className="members-editor";row.innerHTML=`<input type="hidden" class="mid" value="${esc(m.id||"")}"><div class="row"><div class="field"><label>${type==="child"?"🧒 Criança":"👤 Adulto"}</label><input class="mname" data-type="${type}" value="${esc(m.name||"")}" placeholder="Nome"></div><div class="field"><label>Situação</label><select class="mstatus"><option value="pending" ${!m.attendance_status||m.attendance_status==="pending"?"selected":""}>Aguardando</option><option value="yes" ${m.attendance_status==="yes"?"selected":""}>Vai</option><option value="no" ${m.attendance_status==="no"?"selected":""}>Não vai</option></select></div></div><button type="button" class="btn danger small rm">Remover</button>`;row.querySelector(".mname").oninput=()=>{if(root.firstElementChild===row)touched=true};row.querySelector(".rm").onclick=()=>row.remove();root.append(row)};
 (guest?.members||[]).forEach(m=>add(m.person_type,m));if(!guest)add("adult",{attendance_status:"pending"});
 primary.oninput=()=>{const first=root.firstElementChild;if(first){const i=first.querySelector(".mname");if(!touched||!i.value.trim())i.value=primary.value}};
 w.querySelector("#aa").onclick=()=>add("adult",{attendance_status:"pending"});w.querySelector("#ac").onclick=()=>add("child",{attendance_status:"pending"});
 form.onsubmit=async ev=>{ev.preventDefault();const d=new FormData(form),members=[...root.children].map(r=>({id:r.querySelector(".mid").value||undefined,name:r.querySelector(".mname").value.trim(),person_type:r.querySelector(".mname").dataset.type,attendance_status:r.querySelector(".mstatus").value,is_preapproved:true})).filter(m=>m.name);if(!members.length)return toast("Cadastre pelo menos uma pessoa.",true);const status=members.some(m=>m.attendance_status==="yes")?"yes":members.some(m=>m.attendance_status==="pending")?"pending":"no",base=role==="admin"?`/api/admin/events/${eventId}`:`/api/client/${encodeURIComponent(token)}`,url=guest?`${base}/guests/${guest.id}`:`${base}/guests`,b=ev.submitter;b.disabled=true;try{const r=await api(url,{method:guest?"PATCH":"POST",body:JSON.stringify({group_label:d.get("group_label"),primary_name:d.get("primary_name"),phone:d.get("phone"),max_people_allowed:d.get("max_people_allowed"),dietary:d.get("dietary"),notes:d.get("notes"),love_message:d.get("love_message"),members,response_status:status})});w.closeModal();toast(r.duplicate_matches?.length?`Salvo. Possível duplicidade: ${[...new Set(r.duplicate_matches.map(x=>x.name))].join(", ")}.`:"Salvo.");onSaved?.()}catch(e){toast(e.message,true);b.disabled=false}};
}
...
async function messagesTab({root,event,role,eventId,token}){
 if(role==="client"&&!safePerms(event).view_messages){root.innerHTML=`<div class="empty">Mensagens não liberadas para este evento.</div>`;return}
 const base=role==="admin"?`/api/admin/events/${eventId}`:`/api/client/${encodeURIComponent(token)}`;
 root.innerHTML=`<div class="section-title"><div><h2>Mensagens 💌</h2><span class="meta">Recadinhos dos convidados em um lugar só.</span></div></div><div class="toolbar"><div class="search-shell"><input id="ms" placeholder="Buscar..."></div></div><div id="ml">${loading()}</div>`;let timer;
 const refresh=async(q="")=>{const d=await api(`${base}/messages?q=${encodeURIComponent(q)}`);root.querySelector("#ml").innerHTML=d.messages.length?`<div class="message-grid">${d.messages.map(m=>`<article class="message-card"><blockquote>“${esc(m.message)}”</blockquote><footer><strong>${esc(m.name)}</strong> • ${esc(fmtDT(m.responded_at))}</footer></article>`).join("")}</div>`:`<div class="empty">Ainda não há mensagens carinhosas.</div>`};
...
async function appearanceTab({root,event,role,eventId,token,allowAppearance,allowTexts}){
 ...
 if(allowAppearance){
  ...
  root.querySelectorAll("[data-upload]").forEach(i=>i.onchange=async()=>{const file=i.files?.[0];if(!file)return;if(appearanceDirty){i.value="";return toast("Salve as alterações atuais antes de enviar uma nova mídia.",true)}const fd=new FormData();fd.append("file",file);fd.append("kind",i.dataset.upload);const zone=i.closest(".upload-zone"),progress=zone?.querySelector("[data-upload-progress]"),bar=zone?.querySelector("[data-upload-bar]"),pct=zone?.querySelector("[data-upload-percent]");i.disabled=true;zone?.classList.add("uploading");if(progress)progress.hidden=false;if(pct)pct.textContent="0%";if(bar)bar.style.width="0%";try{const r=await uploadWithProgress(`${base}/media`,fd,n=>{if(bar)bar.style.width=`${n}%`;if(pct)pct.textContent=n>=100?"Processando...":`${n}%`});if(bar)bar.style.width="100%";if(pct)pct.textContent="Concluído ✓";toast("Mídia enviada.");setAppearanceDirty(false);setTimeout(()=>appearanceTab({root,event:r.event,role,eventId,token,allowAppearance,allowTexts}),250)}catch(e){toast(e.message,true);i.disabled=false;zone?.classList.remove("uploading");if(progress)progress.hidden=true;i.value=""}});
...
function appearanceControls(e,a,media){
 ...
 return`<section class="card panel" style="margin-bottom:14px"><h3>Fundo</h3><div class="field"><label>Tipo</label><select name="background_type"><option value="none" ${e.background_type==="none"?"selected":""}>Sem mídia</option><option value="image" ${e.background_type==="image"?"selected":""}>Imagem</option><option value="video" ${e.background_type==="video"?"selected":""}>Vídeo em loop</option></select></div><div class="row"><div class="field"><label>Posição vertical</label><select name="background_position"><option value="top" ${a.background_position==="top"?"selected":""}>Topo</option><option value="center" ${a.background_position==="center"?"selected":""}>Centro</option><option value="bottom" ${a.background_position==="bottom"?"selected":""}>Inferior</option></select></div><div class="field"><label>Posição horizontal</label><select name="background_x"><option value="left" ${a.background_x==="left"?"selected":""}>Esquerda</option><option value="center" ${a.background_x==="center"?"selected":""}>Centro</option><option value="right" ${a.background_x==="right"?"selected":""}>Direita</option></select></div></div><div class="grid two">${upload("background_image","Imagem de fundo","JPG, PNG, WebP ou AVIF • até 10 MB","image/jpeg,image/png,image/webp,image/avif")}${upload("background_video","Vídeo de fundo","MP4 ou WebM • até 20 MB","video/mp4,video/webm")}</div>${media.length?`<div class="section-title"><h3>Mídias enviadas</h3></div><div class="grid two">${media.map(mediaHtml).join("")}</div>`:""}</section>
...
async function clientApp(token,requested="overview"){
 ...
 }catch(err){app.innerHTML=`<main class="shell">${topbar()}<section class="card panel"><h1>Acesso indisponível</h1><p>${esc(err.message)}</p></section></main>`}
}
function clientSettings(root,e,token){root.innerHTML=`<section class="card panel panel-narrow"><h3>Dados do evento</h3><form id="cdf"><div class="row"><div class="field"><label>Data</label><input type="date" name="event_date" value="${esc(e.event_date||"")}"></div><div class="field"><label>Horário</label><input type="time" name="event_time" value="${esc(e.event_time||"")}"></div></div><div class="field"><label>Mensagem inicial</label><textarea name="welcome_message">${esc(e.welcome_message||"")}</textarea></div><button class="btn">Salvar</button></form></section>`;root.querySelector("#cdf").onsubmit=async ev=>{ev.preventDefault();const d=new FormData(ev.currentTarget);try{await api(`/api/client/${encodeURIComponent(token)}/event`,{method:"PATCH",body:JSON.stringify({event_date:d.get("event_date"),event_time:d.get("event_time"),welcome_message:d.get("welcome_message")})});toast("Atualizado.");clientApp(token,"settings")}catch(x){toast(x.message,true)}}}

async function publicApp(slug){
 try{
  const d=await api(`/api/public/events/${encodeURIComponent(slug)}`),e=d.event,a=safeAppearance(e),t=safeTexts(e);
  syncPublicViewport();
  document.documentElement.style.setProperty("--brand",a.button_color);
  document.documentElement.style.setProperty("--brand-strong",a.button_color);
  document.documentElement.style.setProperty("--event-primary",a.button_color);
  document.documentElement.style.setProperty("--event-text",a.text_color);
  document.documentElement.style.setProperty("--event-card",rgb(a.card_color));
  document.documentElement.style.setProperty("--event-card-opacity",String(a.card_opacity));
  ...
  const pos=bgPositionCss(a.background_position,a.background_x),media=e.background_type==="video"&&e.background_video_url?`<div class="public-media"><video class="public-bg-video" src="${esc(e.background_video_url)}" autoplay muted loop playsinline preload="metadata" style="object-position:${pos}"></video></div>`:e.background_type==="image"&&e.background_image_url?`<div class="public-media"><img class="public-bg-image" src="${esc(e.background_image_url)}" alt="" style="object-position:${pos}"></div>`:"";
 ...
 }catch(err){app.innerHTML=`<main class="public-page"><section class="public-card"><h1>Convite indisponível</h1><p>${esc(err.message)}</p></section></main>`}
}
function closedPublic(e){const t=safeTexts(e);document.querySelector("#publicFlow").innerHTML=`<div class="success"><div class="bubble">♡</div><h2>${esc(t.closed_title)}</h2><p>${esc(e.closed_reason||"O período de confirmação foi encerrado.")}</p></div>`}

function publicLookup(e){
 const root=document.querySelector("#publicFlow"),t=safeTexts(e);
 root.innerHTML=`<div class="notice" style="margin:16px 0">Digite pelo menos 2 letras. As sugestões aparecem abreviadas para proteger a lista.</div><div class="field public-search"><label>${esc(t.lookup_label)}</label><input id="lookup" autocomplete="off" placeholder="${esc(t.lookup_placeholder)}"><div id="suggestions" class="public-search-results" style="display:none"></div></div>`;
 const input=root.querySelector("#lookup"),box=root.querySelector("#suggestions");
 input.oninput=()=>{const q=input.value.trim();clearTimeout(suggestTimer);if(q.length<2){box.style.display="none";return}suggestTimer=setTimeout(async()=>{try{const d=await api(`/api/public/events/${encodeURIComponent(e.slug)}/suggestions?q=${encodeURIComponent(q)}`);box.innerHTML=d.suggestions.length?d.suggestions.map(s=>`<button type="button" class="public-search-result" data-guest="${s.guest_id}"><span class="public-search-avatar">${s.person_type==="child"?"🧒":"👤"}</span><span><strong>${esc(s.display_name)}</strong><small>Toque para abrir sua confirmação</small></span></button>`).join(""):`<div class="search-result"><span class="subtle">Nenhuma sugestão encontrada.</span></div>`;box.style.display="block";box.querySelectorAll("[data-guest]").forEach(b=>b.onclick=async()=>{b.disabled=true;try{const r=await api(`/api/public/events/${encodeURIComponent(e.slug)}/lookup`,{method:"POST",body:JSON.stringify({guest_id:b.dataset.guest})});listRsvp(e,r.guest)}catch(x){toast(x.message,true);b.disabled=false}})}catch(x){toast(x.message,true)}},220)};
}

function listRsvp(e,g){
 const root=document.querySelector("#publicFlow"),f=e.extra_fields||{},t=safeTexts(e),flex=g.list_behavior==="flexible",limit=Number(g.effective_limit||0)||null;
 root.innerHTML=`<div class="public-family"><div class="public-family-title">${esc(g.group_label||g.primary_name)}</div><p class="subtle">Marque individualmente quem poderá comparecer.${flex&&limit?` Até ${limit} pessoa(s) presentes.`:""}</p><div id="existing">${g.members.map(m=>`<div class="public-person" data-member="${m.id}" data-status="${m.attendance_status}"><div><span class="public-person-name">${esc(m.name)}</span><small>${m.person_type==="child"?"Criança":"Adulto"}</small></div><div class="member-attendance"><button type="button" class="yes ${m.attendance_status==="yes"?"active":""}" data-choice="yes">✓ Vai</button><button type="button" class="no ${m.attendance_status==="no"?"active":""}" data-choice="no">Não vai</button></div></div>`).join("")}</div></div>
 ${flex?`<div class="members-editor"><h3>Adicionar alguém</h3><div id="newMembers"></div><div class="actions"><button type="button" class="btn secondary small" id="na">+ Adulto</button><button type="button" class="btn secondary small" id="nc">+ Criança</button></div></div>`:""}
 <form id="lrf"><div style="position:absolute;left:-9999px"><input name="website" autocomplete="off"></div>${f.phone?`<div class="field"><label>Telefone</label><input name="phone" inputmode="tel"></div>`:""}${f.dietary?`<div class="field"><label>Restrição alimentar</label><input name="dietary"></div>`:""}${f.notes?`<div class="field"><label>Observações</label><textarea name="notes"></textarea></div>`:""}${f.love_message!==false?`<div class="field"><label>${esc(t.message_label)}</label><textarea name="love_message" placeholder="${esc(t.message_placeholder)}"></textarea></div>`:""}<button class="btn block large">Enviar confirmação</button><button type="button" class="btn ghost block" id="backLookup">← Procurar outro nome</button></form>`;
 ...
 const addNew=type=>{const confirmed=[...root.querySelectorAll("[data-member]")].filter(r=>r.dataset.status==="yes").length+(newRoot?[...newRoot.children].length:0);if(limit&&confirmed>=limit)return toast(`Limite de ${limit} pessoa(s).`,true);const r=document.createElement("div");r.className="member-editor-row";r.innerHTML=`<div class="member-type-badge">${type==="child"?"🧒 Criança":"👤 Adulto"}</div><input class="newname" data-type="${type}" placeholder="Nome"><button type="button" class="btn danger small rm">×</button>`;r.querySelector(".rm").onclick=()=>r.remove();newRoot.append(r)};
 ...
 listForm.onsubmit=async ev=>{ev.preventDefault();const d=new FormData(ev.currentTarget),responses=[...root.querySelectorAll("[data-member]")].map(r=>({id:r.dataset.member,attendance_status:r.dataset.status||"pending"})),newMembers=flex?[...newRoot.children].map(r=>({name:r.querySelector(".newname").value.trim(),person_type:r.querySelector(".newname").dataset.type,attendance_status:"yes"})).filter(x=>x.name):[],confirmed=responses.filter(x=>x.attendance_status==="yes").length+newMembers.length;if(limit&&flex&&confirmed>limit){scrollToProblem(root.querySelector(".public-family"));return toast(`Limite de ${limit} pessoa(s).`,true)}if(!responses.some(x=>x.attendance_status==="yes"||x.attendance_status==="no")&&!newMembers.length){scrollToProblem(root.querySelector(".public-family"));return toast("Marque quem vai ou não vai.",true)}const b=ev.submitter;b.disabled=true;try{const r=await api(`/api/public/events/${encodeURIComponent(e.slug)}/rsvp`,{method:"POST",body:JSON.stringify({website:d.get("website"),guest_id:g.id,member_responses:responses,new_members:newMembers,phone:d.get("phone"),dietary:d.get("dietary"),notes:d.get("notes"),love_message:d.get("love_message")})}),yes=r.guest.members?.some(m=>m.attendance_status==="yes");success(e,yes?"yes":"no")}catch(x){toast(x.message,true);b.disabled=false}};
}
