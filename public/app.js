...
function guestCards(guests,editable){
 ...
 return`<div class="guest-list">${guests.map(g=>`<article class="guest-card"><div class="guest-card-head"><div><div class="chips"><span class="status ${g.response_status}">${statusLabel(g.response_status)}</span>${g.max_people_allowed?`<span class="chip">limite ${g.max_people_allowed}</span>`:""}</div><h3 class="guest-card-name">${esc(g.group_label||g.primary_name)}</h3>${g.group_label?`<div class="guest-card-group">Contato: ${esc(g.primary_name)}</div>`:""}</div><span class="subtle">${esc(sourceLabel(g.source))}</span></div>
...
async function publicApp(slug){
 try{
  ...
  ${a.logo_url?`<img src="${esc(a.logo_url)}" alt="" style="width:72px;height:72px;object-fit:contain;margin:0 auto 12px">`:""}${a.cover_url?`<img src="${esc(a.cover_url)}" alt="" style="width:100%;max-height:190px;object-fit:cover;border-radius:18px;margin-bottom:16px">`:""}
