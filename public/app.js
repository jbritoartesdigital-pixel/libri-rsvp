...
function translateServerMessage(message,lang=activeInterfaceLanguage){
 ...
 const exact={
  ...
  "Informe o limite de adultos e crianças.":"Enter both the adult and child limits.",
  "O limite de adultos não pode ser menor que os adultos já cadastrados.":"The adult limit cannot be lower than the adults already registered.",
  "O limite de crianças não pode ser menor que as crianças já cadastradas.":"The child limit cannot be lower than the children already registered."
 };
 ...
 m=m.replace(/^Esta confirmação permite no máximo (\d+) adulto\(s\) presentes\.$/,"This RSVP allows up to $1 adults.");
 m=m.replace(/^Esta confirmação permite no máximo (\d+) criança\(s\) presentes\.$/,"This RSVP allows up to $1 children.");
...
function guestCards(guests,editable,L="pt-BR",options={}){
 ...
 return`<div class="guest-list">${guests.map(g=>{
  const hasAdultLimit=g.max_adults_allowed!==null&&g.max_adults_allowed!==undefined&&String(g.max_adults_allowed)!=="";
  const hasChildLimit=g.max_children_allowed!==null&&g.max_children_allowed!==undefined&&String(g.max_children_allowed)!=="";
  const compositionLimit=hasAdultLimit||hasChildLimit;
  const limitChip=compositionLimit
   ?`<span class="chip">${tr(L,"máx.","max.")} ${Number(g.max_adults_allowed||0)}A + ${Number(g.max_children_allowed||0)}C</span>`
   :g.max_people_allowed?`<span class="chip">${tr(L,"limite","limit")} ${g.max_people_allowed}</span>`:"";
  return`<article class="guest-card${isRecentResponse(g.responded_at)?" recent-response":""}${selectedIds.has(g.id)?" selected":""}" data-guest-card="${esc(g.id)}"><div class="guest-card-head"><div class="guest-card-title-wrap">${editable&&selectionMode?`<label class="guest-select"><input type="checkbox" data-select-guest="${esc(g.id)}" aria-label="${tr(L,"Selecionar","Select")} ${esc(g.group_label||g.primary_name)}" ${selectedIds.has(g.id)?"checked":""}><span></span></label>`:""}<div><div class="chips"><span class="status ${g.response_status}">${statusLabel(g.response_status,L)}</span>${isRecentResponse(g.responded_at)?`<span class="chip new-response">${tr(L,"NOVA","NEW")}</span>`:""}${limitChip}${g.possible_duplicate?`<span class="chip duplicate ${g.suggested_keep?"keep":"copy"}">${g.suggested_keep?tr(L,"Sugestão: manter","Suggested: keep"):tr(L,"Possível repetição","Possible copy")}</span>`:""}</div><h3 class="guest-card-name">${esc(g.group_label||g.primary_name)}</h3>${g.group_label?`<div class="guest-card-group">${tr(L,"Contato","Primary contact")}: ${esc(g.primary_name)}</div>`:""}</div></div><span class="subtle">${esc(sourceLabel(g.source,L))}</span></div>
...
function guestModal({event,role,eventId,token,guest=null,onSaved}){
 ...
 const initialLimitMode=hasSavedAdultLimit||hasSavedChildLimit?"composition":"total";
 const membersTitle=isFlexible?tr(L,"Quem já está incluído neste convite?","Who is already included in this invitation?"):isList?tr(L,"Quem está incluído neste convite?","Who is included in this invitation?"):tr(L,"Pessoas deste cadastro","People in this record");
 const membersHelp=isFlexible?tr(L,"Cadastre todas as pessoas que você já souber. O convidado poderá adicionar outras respeitando o limite definido abaixo. Uma pessoa por campo.","Add everyone you already know. The guest can add others while respecting the limit below. One person per field."):isList?tr(L,"Cadastre todas as pessoas desta família. Uma pessoa por campo.","Add every person in this family. One person per field."):tr(L,"Cadastre cada pessoa individualmente. Uma pessoa por campo.","Add each person individually. One person per field.");
 const subtitle=isFlexible?tr(L,"Na lista flexível, você pode limitar só o total ou separar quantas vagas são de adulto e de criança.","On a flexible list, you can limit only the total or set separate adult and child capacities."):isList?tr(L,"Esses nomes poderão ser encontrados na busca do convite.","These names will be searchable on the invitation."):tr(L,"Cadastre cada pessoa individualmente.","Add each person individually.");
 const flexibleLimitFields=isFlexible?`<div class="form-section"><h3>${tr(L,"Limite desta família","Family limit")}</h3><div class="field"><label>${tr(L,"Como deseja limitar?","How should this RSVP be limited?")}</label><select name="limit_mode" id="familyLimitMode"><option value="total" ${initialLimitMode==="total"?"selected":""}>${tr(L,"Somente total de pessoas","Total people only")}</option><option value="composition" ${initialLimitMode==="composition"?"selected":""}>${tr(L,"Separar adultos e crianças","Separate adults and children")}</option></select></div><div id="totalLimitFields"><div class="field"><label>${tr(L,"Máximo de pessoas no total","Maximum people in total")}</label><input name="max_people_allowed" id="familyLimit" type="number" min="1" max="100" placeholder="${event.max_people_per_rsvp?tr(L,`Padrão: ${event.max_people_per_rsvp}`,`Default: ${event.max_people_per_rsvp}`):tr(L,"Sem limite","No limit")}" value="${esc(initialLimitMode==="total"?(guest?.max_people_allowed??""):"")}"><small class="field-help">${tr(L,"Qualquer combinação de adultos e crianças. Inclui quem já está cadastrado.","Any combination of adults and children. Includes people already registered.")}</small></div></div><div id="compositionLimitFields"><div class="row"><div class="field"><label>${tr(L,"Máximo de adultos","Maximum adults")}</label><input name="max_adults_allowed" id="familyAdultLimit" type="number" min="0" max="100" value="${esc(hasSavedAdultLimit?guest.max_adults_allowed:"")}" placeholder="1"></div><div class="field"><label>${tr(L,"Máximo de crianças","Maximum children")}</label><input name="max_children_allowed" id="familyChildLimit" type="number" min="0" max="100" value="${esc(hasSavedChildLimit?guest.max_children_allowed:"")}" placeholder="1"></div></div><div class="notice" style="margin:0"><strong>${tr(L,"Exemplo: 1 adulto + 1 criança","Example: 1 adult + 1 child")}</strong><span>${tr(L,"Uma vaga de criança não vira vaga de adulto, e vice-versa.","A child spot cannot be used by an adult, and vice versa.")}</span></div></div><div id="familyLimitStatus" class="subtle" style="margin-top:10px"></div></div>`:!isList?`<div class="field"><label>${tr(L,"Limite desta família","Family limit")}</label><input name="max_people_allowed" id="familyLimit" type="number" min="1" max="100" placeholder="${event.max_people_per_rsvp?tr(L,`Padrão: ${event.max_people_per_rsvp}`,`Default: ${event.max_people_per_rsvp}`):tr(L,"Sem limite","No limit")}" value="${esc(guest?.max_people_allowed??"")}"><small class="field-help">${tr(L,"Deixe vazio para usar o limite padrão do evento.","Leave blank to use the event default limit.")}</small></div>`:"";
 const w=modal(guest?tr(L,"Editar confirmação","Edit RSVP"):tr(L,"Cadastrar convidado/família","Add guest/family"),`<form id="gf"><div class="field"><label>${tr(L,"Nome da família / grupo","Family / group name")}</label><input name="group_label" placeholder="${tr(L,"Ex.: Família Brito","e.g. Brito Family")}" value="${esc(guest?.group_label||"")}"></div><div class="field"><label>${tr(L,"Contato principal","Primary contact")}</label><input name="primary_name" id="primary" required value="${esc(guest?.primary_name||"")}"></div><div class="field"><label>${tr(L,"Telefone","Phone")}</label><input name="phone" value="${esc(guest?.phone||"")}"></div>${flexibleLimitFields}<div class="form-section"><h3>${membersTitle}</h3><p class="subtle" style="margin-bottom:12px">${membersHelp}</p><div id="members"></div><div class="actions"><button type="button" class="btn secondary small" id="aa">＋ ${tr(L,"Adicionar um adulto","Add an adult")}</button><button type="button" class="btn secondary small" id="ac">＋ ${tr(L,"Adicionar uma criança","Add a child")}</button></div></div><details><summary>${tr(L,"Informações opcionais","Optional information")}</summary><div style="margin-top:10px"><div class="field"><label>${tr(L,"Restrição alimentar","Dietary restrictions")}</label><input name="dietary" value="${esc(guest?.dietary||"")}"></div><div class="field"><label>${tr(L,"Observações","Notes")}</label><textarea name="notes">${esc(guest?.notes||"")}</textarea></div></div></details><button class="btn block large" style="margin-top:16px">${guest?tr(L,"Salvar alterações","Save changes"):tr(L,"Cadastrar família","Add family")}</button></form>`,subtitle,true);
 const form=w.querySelector("#gf"),root=w.querySelector("#members"),primary=w.querySelector("#primary"),limitMode=w.querySelector("#familyLimitMode"),limitInput=w.querySelector("#familyLimit"),adultLimitInput=w.querySelector("#familyAdultLimit"),childLimitInput=w.querySelector("#familyChildLimit"),totalLimitFields=w.querySelector("#totalLimitFields"),compositionLimitFields=w.querySelector("#compositionLimitFields"),limitStatus=w.querySelector("#familyLimitStatus"),addAdult=w.querySelector("#aa"),addChild=w.querySelector("#ac");let touched=false;
 const mode=()=>isFlexible?(limitMode?.value||"total"):"total";
 const totalLimit=()=>Number(limitInput?.value||0)||Number(event.max_people_per_rsvp||0)||null;
 const adultLimit=()=>mode()==="composition"&&adultLimitInput?.value!==""?Number(adultLimitInput.value):null;
 const childLimit=()=>mode()==="composition"&&childLimitInput?.value!==""?Number(childLimitInput.value):null;
 const countType=type=>[...root.querySelectorAll(".mname")].filter(i=>i.dataset.type===type&&i.value.trim()).length;
 const rowCountType=type=>[...root.querySelectorAll(".mname")].filter(i=>i.dataset.type===type).length;
 const updateLimitStatus=()=>{
  if(isFlexible){
   const composition=mode()==="composition";
   if(totalLimitFields)totalLimitFields.style.display=composition?"none":"";
   if(compositionLimitFields)compositionLimitFields.style.display=composition?"":"none";
   if(composition){
    const a=adultLimit(),c=childLimit(),usedA=rowCountType("adult"),usedC=rowCountType("child");
    if(limitStatus)limitStatus.textContent=a===null||c===null?tr(L,"Informe o máximo de adultos e de crianças.","Enter both the adult and child limits."):tr(L,`${usedA}/${a} adulto(s) • ${usedC}/${c} criança(s) cadastrados.`,`${usedA}/${a} adults • ${usedC}/${c} children registered.`);
    if(addAdult)addAdult.disabled=a!==null&&usedA>=a;
    if(addChild)addChild.disabled=c!==null&&usedC>=c;
   }else{
    const limit=totalLimit(),count=root.children.length;
    if(limitStatus)limitStatus.textContent=limit?tr(L,`${count} de ${limit} pessoa(s) cadastrada(s).`,`${count} of ${limit} people registered.`):tr(L,`${count} pessoa(s) cadastrada(s) • sem limite.`,`${count} people registered • no limit.`);
    const full=!!limit&&count>=limit;if(addAdult)addAdult.disabled=full;if(addChild)addChild.disabled=full;
   }
  }
 };
 const canAddType=type=>{
  ...
  if(mode()==="composition"){
   const lim=type==="child"?childLimit():adultLimit();
   if(lim===null)return true;
   return rowCountType(type)<lim;
  }
  const lim=totalLimit();
  return !lim||root.children.length<lim;
 };
 const add=(type,m={})=>{
  if(!m.id&&!canAddType(type)){const label=type==="child"?tr(L,"crianças","children"):tr(L,"adultos","adults");return toast(tr(L,`O limite de ${label} desta família já foi atingido.`,`This family's ${label} limit has already been reached.`),true)}
  const row=document.createElement("div");row.className="members-editor";row.innerHTML=`<input type="hidden" class="mid" value="${esc(m.id||"")}"><div class="row"><div class="field"><label>${type==="child"?`🧒 ${tr(L,"Criança","Child")}`:`👤 ${tr(L,"Adulto","Adult")}`}</label><input class="mname" data-type="${type}" value="${esc(m.name||"")}" placeholder="${type==="child"?tr(L,"Nome completo da criança","Child's full name"):tr(L,"Nome completo do adulto","Adult's full name")}"></div><div class="field"><label>${tr(L,"Situação","Status")}</label><select class="mstatus"><option value="pending" ${!m.attendance_status||m.attendance_status==="pending"?"selected":""}>${tr(L,"Aguardando","Pending")}</option><option value="yes" ${m.attendance_status==="yes"?"selected":""}>${tr(L,"Vai","Attending")}</option><option value="no" ${m.attendance_status==="no"?"selected":""}>${tr(L,"Não vai","Not attending")}</option></select></div></div><button type="button" class="btn danger small rm">${tr(L,"Remover","Remove")}</button>`;row.querySelector(".mname").oninput=()=>{if(root.firstElementChild===row)touched=true;updateLimitStatus()};row.querySelector(".rm").onclick=()=>{row.remove();updateLimitStatus()};root.append(row);updateLimitStatus()
 };
 ...
 addAdult.onclick=()=>add("adult");addChild.onclick=()=>add("child");
 if(limitMode)limitMode.onchange=updateLimitStatus;if(limitInput)limitInput.oninput=updateLimitStatus;if(adultLimitInput)adultLimitInput.oninput=updateLimitStatus;if(childLimitInput)childLimitInput.oninput=updateLimitStatus;updateLimitStatus();
 form.onsubmit=async ev=>{
  ...
  if(isFlexible&&mode()==="composition"){
   ...
   const registeredA=countType("adult"),registeredC=countType("child");
   if(registeredA>a){scrollToProblem(adultLimitInput);return toast(tr(L,"O limite de adultos não pode ser menor que os adultos já cadastrados.","The adult limit cannot be lower than the adults already registered."),true)}
   if(registeredC>c){scrollToProblem(childLimitInput);return toast(tr(L,"O limite de crianças não pode ser menor que as crianças já cadastradas.","The child limit cannot be lower than the children already registered."),true)}
   body.max_adults_allowed=a;body.max_children_allowed=c;body.max_people_allowed=a+c;
  }else{
   const limit=totalLimit();
   if(isFlexible&&limit&&members.length>limit){scrollToProblem(limitInput);return toast(tr(L,`Cadastre no máximo ${limit} pessoa(s) nesta família. O total inclui quem já está na lista.`,`Add at most ${limit} people to this family. The total includes those already on the list.`),true)}
   if(limitInput)body.max_people_allowed=d.get("max_people_allowed");
...
function listRsvp(e,g){
 const root=document.querySelector("#publicFlow"),f=e.extra_fields||{},t=safeTexts(e),L=eventLang(e),flex=g.list_behavior==="flexible";
 const hasAdultLimit=g.max_adults_allowed!==null&&g.max_adults_allowed!==undefined&&String(g.max_adults_allowed)!=="";
 const hasChildLimit=g.max_children_allowed!==null&&g.max_children_allowed!==undefined&&String(g.max_children_allowed)!=="";
 const composition=flex&&(hasAdultLimit||hasChildLimit),adultLimit=composition?Number(g.max_adults_allowed||0):null,childLimit=composition?Number(g.max_children_allowed||0):null,limit=composition?adultLimit+childLimit:(Number(g.effective_limit||0)||null);
 root.innerHTML=`<div class="public-family"><div class="public-family-title">${esc(g.group_label||g.primary_name)}</div><p class="subtle">${tr(L,"Marque individualmente quem poderá comparecer.","Select each person who will attend.")}</p>${flex&&(limit||composition)?`<div id="limitStatus" class="notice" style="margin:10px 0 12px"></div>`:""}<div id="existing">${g.members.map(m=>`<div class="public-person" data-member="${m.id}" data-type="${m.person_type}" data-status="${m.attendance_status}"><div><span class="public-person-name">${esc(m.name)}</span><small>${m.person_type==="child"?tr(L,"Criança","Child"):tr(L,"Adulto","Adult")}</small></div><div class="member-attendance"><button type="button" class="yes ${m.attendance_status==="yes"?"active":""}" data-choice="yes">✓ ${tr(L,"Vai","Attending")}</button><button type="button" class="no ${m.attendance_status==="no"?"active":""}" data-choice="no">${tr(L,"Não vai","Not attending")}</button></div></div>`).join("")}</div></div>
 ${flex?`<div class="members-editor"><h3>${tr(L,"Adicionar alguém","Add someone")}</h3><p class="subtle">${composition?tr(L,"Adicione uma pessoa por campo. As vagas de adulto e criança são separadas.","Add one person per field. Adult and child spots are separate."):tr(L,"Adicione uma pessoa por campo. As pessoas já cadastradas também contam dentro do limite.","Add one person per field. People already registered also count toward the limit.")}</p><div id="newMembers"></div><div class="actions"><button type="button" class="btn secondary small" id="na">＋ ${tr(L,"Adicionar um adulto","Add an adult")}</button><button type="button" class="btn secondary small" id="nc">＋ ${tr(L,"Adicionar uma criança","Add a child")}</button></div></div>`:""}
 ...
 const newRoot=flex?root.querySelector("#newMembers"):null,limitStatus=flex?root.querySelector("#limitStatus"):null,addAdult=flex?root.querySelector("#na"):null,addChild=flex?root.querySelector("#nc"):null;
 const yesExistingCount=type=>[...root.querySelectorAll("[data-member]")].filter(r=>r.dataset.status==="yes"&&(!type||r.dataset.type===type)).length;
 const reservedNewCount=type=>newRoot?[...newRoot.querySelectorAll(".newname")].filter(i=>!type||i.dataset.type===type).length:0;
 const usedSlots=type=>yesExistingCount(type)+reservedNewCount(type);
 const updateLimitUI=()=>{
  if(!flex)return;
  const used=usedSlots(),usedA=usedSlots("adult"),usedC=usedSlots("child");
  if(limitStatus){
   if(composition){
    const remainingA=Math.max(0,adultLimit-usedA),remainingC=Math.max(0,childLimit-usedC);
    limitStatus.innerHTML=`<strong>${tr(L,`${used} de ${limit} vaga(s) usada(s)`,`${used} of ${limit} spots used`)}</strong><span>${tr(L,`Adultos: ${usedA}/${adultLimit} • Crianças: ${usedC}/${childLimit}`,`Adults: ${usedA}/${adultLimit} • Children: ${usedC}/${childLimit}`)}</span><span class="subtle" style="display:block;margin-top:3px">${tr(L,`Restam ${remainingA} vaga(s) de adulto e ${remainingC} de criança.`,`Remaining: ${remainingA} adult and ${remainingC} child spots.`)}</span>`;
   }else if(limit){
    const remaining=Math.max(0,limit-used);
    limitStatus.innerHTML=`<strong>${tr(L,`${used} de ${limit} vaga(s) usada(s)`,`${used} of ${limit} spots used`)}</strong><span>${tr(L,remaining===1?"Resta 1 vaga.":`Restam ${remaining} vagas.`,remaining===1?"1 spot remaining.":`${remaining} spots remaining.`)}</span>`;
   }
  }
  if(addAdult)addAdult.disabled=composition?usedA>=adultLimit:!!limit&&used>=limit;
  if(addChild)addChild.disabled=composition?usedC>=childLimit:!!limit&&used>=limit;
 };
 const canConfirmType=type=>{
  if(!flex)return true;
  if(limit&&usedSlots()>=limit)return false;
  if(composition){
   const typeLimit=type==="child"?childLimit:adultLimit;
   return usedSlots(type)<typeLimit;
  }
  return true;
 };
 root.querySelectorAll("[data-member]").forEach(row=>row.querySelectorAll("[data-choice]").forEach(b=>b.onclick=()=>{
  const next=b.dataset.choice,current=row.dataset.status||"pending",type=row.dataset.type;
  if(next==="yes"&&current!=="yes"&&!canConfirmType(type)){
   const msg=composition?(type==="child"?tr(L,`O limite é de ${childLimit} criança(s). Uma vaga de adulto não pode ser usada por criança.`,`The limit is ${childLimit} children. An adult spot cannot be used by a child.`):tr(L,`O limite é de ${adultLimit} adulto(s). Uma vaga de criança não pode ser usada por adulto.`,`The limit is ${adultLimit} adults. A child spot cannot be used by an adult.`)):tr(L,`O limite desta confirmação é ${limit} pessoa(s), contando quem já está cadastrado.`,`This RSVP is limited to ${limit} people, including pre-registered guests.`);
   return toast(msg,true)
  }
  row.querySelectorAll("[data-choice]").forEach(x=>x.classList.remove("active"));b.classList.add("active");row.dataset.status=next;updateLimitUI()
 }));
 const addNew=type=>{
  if(!newRoot)return;
  if(!canConfirmType(type)){
   const msg=composition?(type==="child"?tr(L,`Todas as ${childLimit} vaga(s) de criança já estão ocupadas.`,`All ${childLimit} child spots are already in use.`):tr(L,`Todas as ${adultLimit} vaga(s) de adulto já estão ocupadas.`,`All ${adultLimit} adult spots are already in use.`)):tr(L,`Todas as ${limit} vagas desta confirmação já estão ocupadas.`,`All ${limit} spots on this RSVP are already in use.`);
   return toast(msg,true)
  }
  const r=document.createElement("div");r.className="member-editor-row";r.innerHTML=`<div class="member-type-badge">${type==="child"?`🧒 ${tr(L,"Criança","Child")}`:`👤 ${tr(L,"Adulto","Adult")}`}</div><input class="newname" data-type="${type}" placeholder="${type==="child"?tr(L,"Nome completo da criança","Child's full name"):tr(L,"Nome completo do adulto","Adult's full name")}"><button type="button" class="btn danger small rm">×</button>`;r.querySelector(".rm").onclick=()=>{r.remove();updateLimitUI()};newRoot.append(r);updateLimitUI()
 };
 if(flex){addAdult.onclick=()=>addNew("adult");addChild.onclick=()=>addNew("child");updateLimitUI()}
 root.querySelector("#backLookup").onclick=()=>publicLookup(e);
 const listForm=root.querySelector("#lrf");bindInvalidScroll(listForm);
 listForm.onsubmit=async ev=>{
  ev.preventDefault();
  const d=new FormData(ev.currentTarget),responses=[...root.querySelectorAll("[data-member]")].map(r=>({id:r.dataset.member,attendance_status:r.dataset.status||"pending"})),newInputs=flex?[...newRoot.querySelectorAll(".newname")]:[],blankInput=newInputs.find(i=>!i.value.trim());
  if(blankInput){scrollToProblem(blankInput);return toast(tr(L,"Preencha ou remova os campos de acompanhante que ficaram vazios.","Fill in or remove any empty companion fields."),true)}
  const newMembers=flex?newInputs.map(i=>({name:i.value.trim(),person_type:i.dataset.type,attendance_status:"yes"})):[];
  const confirmedA=responses.filter(x=>x.attendance_status==="yes"&&g.members.find(m=>m.id===x.id)?.person_type!=="child").length+newMembers.filter(x=>x.person_type==="adult").length;
  const confirmedC=responses.filter(x=>x.attendance_status==="yes"&&g.members.find(m=>m.id===x.id)?.person_type==="child").length+newMembers.filter(x=>x.person_type==="child").length;
  const confirmed=confirmedA+confirmedC;
  if(composition&&confirmedA>adultLimit){scrollToProblem(limitStatus);return toast(tr(L,`Esta confirmação permite no máximo ${adultLimit} adulto(s).`,`This RSVP allows at most ${adultLimit} adults.`),true)}
  if(composition&&confirmedC>childLimit){scrollToProblem(limitStatus);return toast(tr(L,`Esta confirmação permite no máximo ${childLimit} criança(s).`,`This RSVP allows at most ${childLimit} children.`),true)}
...
function freeRsvp(e){
 ...
 root.innerHTML=`<form id="frf"><div style="position:absolute;left:-9999px"><input name="website" autocomplete="off"></div><div class="field"><label>${esc(t.name_label)}</label><input id="fp" name="primary_name" required autocomplete="name" placeholder="${tr(L,"Digite seu nome completo","Enter your full name")}"></div><div class="choice"><button type="button" data-r="yes">${esc(t.yes_button)}</button><button type="button" data-r="no">${esc(t.no_button)}</button></div><div id="declineHint" class="decline-hint" style="display:none">${esc(t.decline_hint)}</div><input type="hidden" name="response_status" value=""><div id="freeSection" style="display:none">${companionGate}</div>${f.phone?`<div class="field"><label>${tr(L,"Telefone","Phone")}</label><input name="phone"></div>`:""}${f.dietary?`<div id="attendeeOnlyDietary" class="attendee-only" style="display:none"><div class="field"><label>${tr(L,"Restrição alimentar","Dietary restrictions")}</label><input name="dietary"></div></div>`:""}${f.notes?`<div class="field"><label>${tr(L,"Observações","Notes")}</label><textarea name="notes"></textarea></div>`:""}${f.love_message!==false?`<div class="field"><label>${esc(t.message_label)}</label><textarea name="love_message" placeholder="${esc(t.message_placeholder)}"></textarea></div>`:""}<button class="btn block large">${tr(L,"Enviar confirmação","Submit RSVP")}</button></form>`;
 const form=root.querySelector("#frf"),status=form.querySelector('[name="response_status"]'),declineHint=root.querySelector("#declineHint"),attendeeDietary=root.querySelector("#attendeeOnlyDietary"),section=root.querySelector("#freeSection"),mr=root.querySelector("#freeMembers"),companionStatus=form.querySelector('[name="has_companion"]'),companionSection=root.querySelector("#companionSection");
 const add=type=>{if(!mr)return;if(limit&&1+mr.children.length>=limit)return toast(tr(L,`Limite de ${limit} pessoa(s).`,`Limit of ${limit} people.`),true);const r=document.createElement("div");r.className="member-editor-row";r.innerHTML=`<div class="member-type-badge">${type==="child"?`🧒 ${tr(L,"Criança","Child")}`:`👤 ${tr(L,"Adulto","Adult")}`}</div><input class="fname" data-type="${type}" placeholder="${type==="child"?tr(L,"Nome completo da criança","Child's full name"):tr(L,"Nome completo do adulto","Adult's full name")}"><button type="button" class="btn danger small rm">×</button>`;r.querySelector(".rm").onclick=()=>r.remove();mr.append(r)};
 const adultButton=root.querySelector("#fa"),childButton=root.querySelector("#fc");if(adultButton)adultButton.onclick=()=>add("adult");if(childButton)childButton.onclick=()=>add("child");
