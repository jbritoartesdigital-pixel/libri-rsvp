...
function guestModal({event,role,eventId,token,guest=null,onSaved}){
 ...
 const limitLabel=isFlexible?tr(L,"Máximo de pessoas no total","Maximum people in total"):tr(L,"Limite desta família","Family limit");
 const limitHelp=isFlexible?tr(L,"Inclui todas as pessoas já cadastradas neste convite.","Includes everyone already registered on this invitation."):tr(L,"Deixe vazio para usar o limite padrão do evento.","Leave blank to use the event default limit.");
 ...
 addAdult.onclick=()=>add("adult");addChild.onclick=()=>add("child");if(limitInput)limitInput.oninput=updateLimitStatus;updateLimitStatus();
 form.onsubmit=async ev=>{ev.preventDefault();const d=new FormData(form),members=[...root.children].map(r=>({id:r.querySelector(".mid").value||undefined,name:r.querySelector(".mname").value.trim(),person_type:r.querySelector(".mname").dataset.type,attendance_status:r.querySelector(".mstatus").value,is_preapproved:true})).filter(x=>x.name),limit=effectiveModalLimit();if(limit&&members.length>limit){scrollToProblem(limitInput);return toast(tr(L,`Cadastre no máximo ${limit} pessoa(s) nesta família. O total inclui quem já está na lista.`,`Add at most ${limit} people to this family. The total includes those already on the list.`),true)}const body={group_label:d.get("group_label"),primary_name:d.get("primary_name"),phone:d.get("phone"),dietary:d.get("dietary"),notes:d.get("notes"),members};if(limitInput)body.max_people_allowed=d.get("max_people_allowed");const base=role==="admin"?`/api/admin/events/${eventId}`:`/api/client/${encodeURIComponent(token)}`;try{await api(guest?`${base}/guests/${guest.id}`:`${base}/guests`,{method:guest?"PATCH":"POST",body:JSON.stringify(body)});w.closeModal();toast(guest?tr(L,"Alterações salvas.","Changes saved."):tr(L,"Convidado cadastrado.","Guest added."));onSaved?.()}catch(e){toast(e.message,true)}};
}
...
function listRsvp(e,g){
 ...
 const yesExistingCount=()=>[...root.querySelectorAll("[data-member]")].filter(r=>r.dataset.status==="yes").length;
 const reservedNewCount=()=>newRoot?[...newRoot.children].length:0;
 const usedSlots=()=>yesExistingCount()+reservedNewCount();
 const updateLimitUI=()=>{if(!flex||!limit)return;const used=usedSlots(),remaining=Math.max(0,limit-used);limitStatus.innerHTML=`<strong>${tr(L,`${used} de ${limit} vaga(s) usada(s)`,`${used} of ${limit} spots used`)}</strong><span>${tr(L,remaining===1?"Resta 1 vaga.":`Restam ${remaining} vagas.`,remaining===1?"1 spot remaining.":`${remaining} spots remaining.`)}</span>`;const full=used>=limit;if(addAdult)addAdult.disabled=full;if(addChild)addChild.disabled=full};
 root.querySelectorAll("[data-member]").forEach(row=>row.querySelectorAll("[data-choice]").forEach(b=>b.onclick=()=>{const next=b.dataset.choice,current=row.dataset.status||"pending";if(flex&&limit&&next==="yes"&&current!=="yes"&&usedSlots()>=limit){return toast(tr(L,`O limite desta confirmação é ${limit} pessoa(s), contando quem já está cadastrado.`,`This RSVP is limited to ${limit} people, including pre-registered guests.`),true)}row.querySelectorAll("[data-choice]").forEach(x=>x.classList.remove("active"));b.classList.add("active");row.dataset.status=next;updateLimitUI()}));
 const addNew=type=>{if(!newRoot)return;if(limit&&usedSlots()>=limit)return toast(tr(L,`Todas as ${limit} vagas desta confirmação já estão ocupadas.`,`All ${limit} spots on this RSVP are already in use.`),true);const r=document.createElement("div");r.className="member-editor-row";r.innerHTML=`<div class="member-type-badge">${type==="child"?`🧒 ${tr(L,"Criança","Child")}`:`👤 ${tr(L,"Adulto","Adult")}`}</div><input class="newname" data-type="${type}" placeholder="${type==="child"?tr(L,"Nome completo da criança","Child's full name"):tr(L,"Nome completo do adulto","Adult's full name")}"><button type="button" class="btn danger small rm">×</button>`;r.querySelector(".rm").onclick=()=>{r.remove();updateLimitUI()};newRoot.append(r);updateLimitUI()};
 ...
 listForm.onsubmit=async ev=>{ev.preventDefault();const d=new FormData(ev.currentTarget),responses=[...root.querySelectorAll("[data-member]")].map(r=>({id:r.dataset.member,attendance_status:r.dataset.status||"pending"})),newInputs=flex?[...newRoot.querySelectorAll(".newname")]:[],blankInput=newInputs.find(i=>!i.value.trim());if(blankInput){scrollToProblem(blankInput);return toast(tr(L,"Preencha ou remova os campos de acompanhante que ficaram vazios.","Fill in or remove any empty companion fields."),true)}const newMembers=flex?newInputs.map(i=>({name:i.value.trim(),person_type:i.dataset.type,attendance_status:"yes"})):[],confirmed=responses.filter(x=>x.attendance_status==="yes").length+newMembers.length;if(limit&&flex&&confirmed>limit){scrollToProblem(root.querySelector("#limitStatus"));return toast(tr(L,`Limite de ${limit} pessoa(s), contando quem já está cadastrado.`,`Limit of ${limit} people, including pre-registered guests.`),true)}if(!responses.some(x=>x.attendance_status==="yes"||x.attendance_status==="no")&&!newMembers.length){scrollToProblem(root.querySelector(".public-family"));return toast(tr(L,"Marque quem vai ou não vai.","Select who is attending or not attending."),true)}const b=ev.submitter;b.disabled=true;try{const r=await api(`/api/public/events/${encodeURIComponent(e.slug)}/rsvp`,{method:"POST",body:JSON.stringify({website:d.get("website"),guest_id:g.id,member_responses:responses,new_members:newMembers,phone:d.get("phone"),dietary:d.get("dietary"),notes:d.get("notes"),love_message:d.get("love_message")})}),yes=r.guest.members?.some(m=>m.attendance_status==="yes");success(e,yes?"yes":"no")}catch(x){toast(x.message,true);b.disabled=false}};
}
function freeRsvp(e){
