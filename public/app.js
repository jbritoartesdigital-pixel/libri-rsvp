...
async function publicApp(slug){
 try{
  ...
  postRsvpEmbedMessage("libri-rsvp-ready",{slug:e.slug});
  if(!e.accepting_rsvp)return closedPublic(e);
  e.rsvp_mode==="list"?publicLookup(e):freeRsvp(e);
 }catch(err){
  postRsvpEmbedMessage("libri-rsvp-error",{slug,error:String(err?.message||err||"Erro ao carregar RSVP")});
  app.innerHTML=`<main class="public-page"><section class="public-card"><h1>${tr(activeInterfaceLanguage,"Convite indisponível","Invitation unavailable")}</h1><p>${esc(err.message)}</p></section></main>`
 }
}
...
function success(e,status){
 ...
 const back=RSVP_EMBED_MODE
  ?`<button class="btn ghost block" type="button" id="successBack">← ${esc(t.back_button)}</button>`
  :a.invitation_url
 ...
 const backBtn=document.querySelector("#successBack");
 if(backBtn){
  backBtn.onclick=()=>{
   postRsvpEmbedMessage("libri-rsvp-close",{
    slug:e.slug,
    status
   });
  };
 ...
 postRsvpEmbedMessage("libri-rsvp-complete",{
  slug:e.slug,
  status
 });
}
